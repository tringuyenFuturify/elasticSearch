package io.futurify.authentication.common;
    ...
    mapOld.forEach((k, v) -> {
      try {
        if (mapNew.get(k) != null) {
          ...
          Field field = null;
          try {
            field = classType.getDeclaredField(snakeCaseToPascalCase(k));
          } catch (NoSuchFieldException e) {
            field = classType.getSuperclass().getDeclaredField(snakeCaseToPascalCase(k));
          }
